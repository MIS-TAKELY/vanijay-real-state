...
export class CmsService {
  ...
  async upsertItem(
    ...
    await this.audit(
      ...
      'content_item',
      item.id,
      `Upserted ${slot} ${placement} '${dto.key}'`,
    );
    return item;
  }
  ...
  async updateItem(
    actorId: string,
    id: string,
    patch: Partial<UpsertContentItemDto>,
  ) {
  ...
  async setPublished(actorId: string, id: string, published: boolean) {
    ...
    await this.audit(
      ...
      'content_item',
      id,
      `${published ? 'Published' : 'Unpublished'} ${item.slot} '${item.key}'`,
    );
    return item;
  }
  ...
  async reorder(
    ...
    await this.audit(
      ...
      `content_item:${slot}`,
      placement,
      `Reordered ${ids.length} items in ${slot}`,
    );
    return result;
  }
  ...
  async upsertStaticPage(
    ...
    await this.audit(
      ...
      'static_page',
      page.id,
      `Upserted static page '${dto.slug}'`,
    );
    return page;
  }

  async listNav() {
    return this.prisma.navItem.findMany({ orderBy: { sortOrder: 'asc' } });
  }
  async upsertNav(
    actorId: string,
    dto: {
  ...
  async upsertFooter(
    ...
    await this.audit(
      ...
      'footer_link',
      link.id,
      `Upserted footer link '${link.label}'`,
    );
    return link;
  }

  async listSeo() {
    return this.prisma.seoConfig.findMany({ orderBy: { route: 'asc' } });
  }
  async upsertSeo(
    actorId: string,
    dto: {
    ...
    const seo = await this.prisma.seoConfig.upsert({
