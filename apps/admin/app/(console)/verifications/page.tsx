  ...
  return (
    <>
      ...
      <section className="mt-lg">
        <AdminDataTable
          minWidth={720}
          columns={["Code", "Title", "Type", "Price", "Owner", "Location", "Submitted"]}
          empty={rows.length === 0}
          emptyMessage="No listings in the verification queue."
        >
          {rows.map((r) => (
            <AdminDataTable.Row key={r.id}>
              <AdminDataTable.Cell className="mono-stat text-[12px] text-on-surface-variant">{r.listingCode}</AdminDataTable.Cell>
              <AdminDataTable.Cell className="font-medium text-on-surface">{r.title}</AdminDataTable.Cell>
              ...
              <AdminDataTable.Cell className="text-on-surface-variant">{[r.location?.district, r.location?.municipality].filter(Boolean).join(", ") || "—"}</AdminDataTable.Cell>
              <AdminDataTable.Cell className="mono-stat text-[12px] text-on-surface-variant">{new Date(r.createdAt).toLocaleDateString()}</AdminDataTable.Cell>
            </AdminDataTable.Row>
          ))}
        </AdminDataTable>
      </section>
