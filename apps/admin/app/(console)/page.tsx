...
async function loadOverview(
  cookie?: string,
): Promise<{ ok: boolean; overview?: AdminOverview; audits?: AuditRow[] }> {
  try {
    ...
    ? [
        {
          ...
          hint:
            String(
              overview!.propertiesByStatus.find((s) => s.status === "LIVE")
                ?._count._all ?? 0,
            ) + " live now",
          icon: "list_alt",
        ...
        },
        {
          ...
          value: String(
            overview!.propertiesByStatus.find(
              (s) => s.status === "UNDER_VERIFICATION",
            )?._count._all ?? 0,
          ),
          hint: "in queue",
          icon: "verified",
        ...
        },
        {
          ...
          hint: overview!.totalSellers + " sellers",
          icon: "manage_accounts",
          tone: "surface" as const,
        },
        {
          label: "Docs Expiring",
          value: String(overview!.documentsExpiring),
          hint: "next 30 days",
          icon: "article",
          tone: "amber" as const,
        },
      ...
      {rows.map((row) => (
        <div key={row.id} className="flex items-start gap-sm py-sm">
          <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-secondary-container text-primary">
            <span className="font-headline-md text-[11px] font-bold">
              {(row.actor?.name || "A").charAt(0).toUpperCase()}
            </span>
          </span>
          <div className="min-w-0 flex-1">
            <p className="truncate text-sm font-medium text-on-surface">
              {row.summary || `${row.action} ${row.entity}`}
            </p>
            <p className="font-label-sm text-[11px] text-on-surface-variant">
              by {row.actor?.name || row.actor?.email} ·{" "}
    ...
    <section className="mt-lg">
      <div className="admin-surface border border-outline-variant rounded-xl p-md">
        ...
        <p className="mt-xs font-body-md text-body-md text-on-surface-variant">
          The API could not be reached (or your session is not authorized).
          Showing demo cards. Start the API and sign in to see live archive
          data.
        </p>
      </div>
    </section>
