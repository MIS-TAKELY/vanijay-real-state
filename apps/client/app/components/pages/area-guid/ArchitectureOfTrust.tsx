      ...
      <div className="mx-auto max-w-container-max px-gutter py-xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {pillars.map((p, i) => (
            <div
              key={p.title}
              className="group bg-surface border border-outline-variant hover:border-primary/30 hover:shadow-lg transition-all duration-300 hover:-translate-y-1 p-6 text-left"
              style={{
                animation: `fadeIn 0.5s ease-out ${i * 0.15}s both`,
              }}
            >
              {/* Icon */}
              <div className="mb-4 flex h-12 w-12 items-center justify-center bg-secondary-container text-primary group-hover:bg-primary group-hover:text-white transition-all duration-300">
                <Icon
