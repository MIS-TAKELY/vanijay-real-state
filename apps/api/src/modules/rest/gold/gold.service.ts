...
export class GoldService {
  ...
  async upsertMetal(actorId: string, dto: MetalConfigDto) {
    ...
    await this.audit(
      ...
      'metal_config',
      metal.id,
      `Upserted metal '${dto.slug}'`,
    );
    return metal;
  }

  async setMetalEnabled(actorId: string, slug: string, isEnabled: boolean) {
    const metal = await this.prisma.metalConfig.update({
      where: { slug },
      data: { isEnabled },
    });
    await this.audit(
      ...
      'metal_config',
      metal.id,
      `${isEnabled ? 'Enabled' : 'Disabled'} metal '${slug}'`,
    );
    return metal;
  }
  ...
  async setFaqs(actorId: string, slug: string, faqs: MetalFaqDto[]) {
    return this.prisma.$transaction(async (tx) => {
      ...
      for (let i = 0; i < faqs.length; i++) {
        await tx.metalFaq.create({
          data: {
            metalId: metal.id,
            question: faqs[i].question,
            answer: faqs[i].answer,
            sortOrder: faqs[i].sortOrder ?? i,
          },
        });
      }
      return tx.metalFaq.findMany({
  ...
  async setOverride(actorId: string, dto: PriceOverrideDto) {
    ...
    await this.prisma.goldPriceOverride.updateMany({
      where: { metalSlug: dto.metalSlug, active: true },
      data: { active: false },
    });
    const override = await this.prisma.goldPriceOverride.create({
      data: {
        metalSlug: dto.metalSlug,
        ask: dto.ask as any,
        bid: dto.bid as any,
        unit: dto.unit ?? 'oz',
    ...
    await this.audit(
      ...
      'gold_price_override',
      override.id,
      `Set ${dto.currency} ${dto.ask}/${dto.bid} for ${dto.metalSlug}`,
    );
    return override;
  }

  async clearOverride(actorId: string, id: string) {
    const o = await this.prisma.goldPriceOverride.update({
      where: { id },
      data: { active: false },
    });
    await this.audit(
      ...
      'gold_price_override',
      id,
      `Cleared override for ${o.metalSlug}`,
    );
    return o;
  }
