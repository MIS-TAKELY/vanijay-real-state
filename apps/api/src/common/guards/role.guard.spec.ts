...
describe('RolesGuard', () => {
  it('allows when no @Roles metadata is present', () => {
    ...
    expect(guard.canActivate(createMockContext({ roles: ['BUYER'] }))).toBe(true);
  });
  ...
  it('allows when the user holds at least one required role (array form)', () => {
    ...
    expect(guard.canActivate(createMockContext({ roles: ['SELLER'] }))).toBe(true);
  });

  it('denies when the user holds none of the required roles', () => {
    const guard = new RolesGuard(mockReflector(['ADMIN']));
    expect(guard.canActivate(createMockContext({ roles: ['BUYER'] }))).toBe(false);
  });
