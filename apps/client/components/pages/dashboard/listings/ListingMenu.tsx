 ...
 * Archive / Duplicate. Click-outside + Escape to close.
 */
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open ? (
        <div
          role="menu"
          className="absolute right-0 top-9 z-20 w-48 rounded-xl border border-outline-variant bg-surface py-1 shadow-lg"
        >
          {LISTING_MENU_ITEMS.map((item) => {
            ...
            return item.href ? (
              <a
                ...
                className="flex items-center gap-sm px-md py-2 text-sm text-on-surface hover:bg-surface-container transition-colors"
              >
                {content}
              </a>
            ) : (
