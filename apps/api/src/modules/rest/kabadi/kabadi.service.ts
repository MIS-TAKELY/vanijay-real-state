...
export class KabadiService {
  ...
  async setCategoryPublished(actorId: string, id: string, published: boolean) {
    ...
    await this.audit(
      ...
      'kabadi_category',
      id,
      `${published ? 'Published' : 'Unpublished'} category '${cat.name}'`,
    );
    return cat;
  }
  ...
  async upsertItem(actorId: string, dto: ItemDto) {
    const item = dto.id
      ? await this.prisma.kabadiItem.update({
          where: { id: dto.id },
          data: { ...dto, rate: dto.rate as any },
        })
      : await this.prisma.kabadiItem.create({
          data: { ...dto, rate: dto.rate as any },
        });
    await this.audit(
      actorId,
  ...
  async setRates(actorId: string, dto: SetRatesDto) {
    const result = await this.prisma.$transaction(
      dto.items.map((i) =>
        i.id
          ? this.prisma.kabadiItem.update({
              where: { id: i.id },
              data: { ...i, rate: i.rate as any },
            })
          : this.prisma.kabadiItem.create({
              data: { ...i, rate: i.rate as any },
            }),
      ),
    );
    await this.audit(
      ...
      'kabadi_item',
      null,
      `Bulk-updated ${dto.items.length} rates`,
    );
    return result;
  }
  ...
  async setItemPublished(actorId: string, id: string, published: boolean) {
    ...
    await this.audit(
      ...
      'kabadi_item',
      id,
      `${published ? 'Published' : 'Unpublished'} rate '${item.name}'`,
    );
    return item;
  }
