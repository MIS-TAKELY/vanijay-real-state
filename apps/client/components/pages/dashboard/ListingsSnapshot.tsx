...
import { SNAPSHOT_LISTINGS, STATUS_STYLES, DEFAULT_STATUS_STYLE } from "./constants";
import { EmptyState } from "./EmptyState";

/**
 * My listings snapshot mini-table (DESIGN.md §5.1 right): thumb, code,
 * status chip, views mono, [Manage]. Compact ledger-style table.
 */
    ...
    <div className="rounded-2xl border border-outline-variant bg-surface p-md">
      <div className="flex items-center justify-between mb-sm">
        <h3 className="font-headline-md text-base font-semibold text-on-surface flex items-center gap-xs">
          <Icon name="list_alt" className="text-[20px] text-on-surface-variant" />
          My Listings
        </h3>
        <Link
      ...
      <div className="flex flex-col">
        ...
        {items.map((item) => {
          const status =
            STATUS_STYLES[item.status] ?? DEFAULT_STATUS_STYLE;
          return (
            <div
              key={item.id}
            ...
            >
              ...
              <span className="mono-stat text-[13px] text-on-surface tabular-nums">
                {item.views}
              </span>
              <Link
