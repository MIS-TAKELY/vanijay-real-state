...
const PLACEMENT = "GLOBAL" as const;
...
export default function GlobalCmsPage() {
  ...
  async function load() {
    ...
    try {
      ...
      setError("Could not load global CMS items. Ensure you are signed in and the API is running.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <>
      <PageHeader
        ...
        description="Site-wide banners, footer copy, contact snippets and other cross-placement content."
      />
...
function AddCmsItem({ placement, onAdded }: { placement: string; onAdded: () => void }) {
  ...
  return (
    <div className="admin-surface border border-dashed border-outline-variant rounded-xl p-md flex flex-wrap items-end gap-sm">
      <div className="w-40"><Label>Slot</Label><Input placeholder="banner" value={slot} onChange={(e) => setSlot(e.target.value)} className="bg-surface" /></div>
      <div className="w-48"><Label>Key</Label><Input placeholder="maintenance_notice" value={key} onChange={(e) => setKey(e.target.value)} className="bg-surface" /></div>
