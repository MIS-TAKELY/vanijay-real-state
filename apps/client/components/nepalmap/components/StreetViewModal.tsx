...
import { TILE_URLS, TILE_ATTR } from "../config";
...
export function StreetViewModal({
  ...
  const popup = (
    // Backdrop + flex centering in one element
    <div
      onClick={(e) => {
      ...
      >
        ...
        <div style={{ flex: 1, overflow: "hidden", minHeight: 0 }}>
          {tab === "data" ? (
            ...
            <SatelliteTabContent marker={marker} />
          ) : (
            <StreetViewLauncher
              marker={marker}
...
function SatelliteTabContent({ marker }: { marker: Marker }) {
  const customIcon = useMemo(() => {
    ...
    return L.divIcon({
      ...
      html: `<div style="
        ...
        border-radius: 50%;
        box-shadow: 0 0 16px rgba(74, 222, 128, 0.9), 0 4px 12px rgba(0,0,0,0.6);
      "></div>`,
      iconSize: [22, 22],
      iconAnchor: [11, 11],
    });
  ...
  return (
    <div style={{ height: 340, width: "100%", position: "relative" }}>
      <MapContainer
        ...
        zoomControl={true}
        preferCanvas={true}
        style={{ height: 340, width: "100%", background: "#0a140d" }}
      >
        <SatelliteMapSetup />
        <TileLayer
        ...
        />
        {customIcon && (
          <LeafletMarker position={[marker.lat, marker.lng]} icon={customIcon} />
        )}
      </MapContainer>
    </div>
  );
