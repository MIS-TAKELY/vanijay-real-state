...
function QaTrend({ days }: { days: PlatformHealth["qaActivity"] }) {
  const definition = useMemo(() => {
    ...
    return defineChart({
      marks: [lineY(folded, { x: "date", y: "value", z: "series", strokeWidth: 2 })],
      x: {
        scale: () => scalePoint<string>().padding(0.1),
        axis: { ticks: { count: 5, format: formatTickDate }, tickLabels: { thin: true, fontSize: 9 } },
      },
      y: {
        scale: () => scaleLinear().domain([0, maxValue]).nice(),
        grid: true,
        axis: { ticks: { count: 3, format: (v: number) => formatCompact(v) }, tickLabels: { fontSize: 9, thin: true } },
      },
      color: {
  ...
  return <Chart definition={definition} height={170} initialWidth={380} ariaLabel="Daily Q&A activity" />;
}
...
export function PlatformCharts({ data }: { data?: PlatformHealth }) {
  if (!data) {
    return <p className="font-body-md text-body-md text-on-surface-variant">No platform data yet.</p>;
  }

  const shares = data.sharesByPlatform.map((s) => ({ key: s.platform, value: s.count }));
  const appointments = data.appointmentsByStatus.map((a) => ({ label: a.status.replace(/_/g, " "), value: a.count }));

  return (
    <div className="grid grid-cols-1 gap-lg md:grid-cols-3">
      <div>
        <p className="mb-sm font-label-sm text-[11px] uppercase tracking-widest text-on-surface-variant">Shares by platform</p>
        <Donut data={shares} height={180} ariaLabel="Shares by platform" />
      </div>
      <div>
        <p className="mb-sm font-label-sm text-[11px] uppercase tracking-widest text-on-surface-variant">Appointments by status</p>
        <HorizontalBars
          data={appointments}
      ...
      <div>
        <p className="mb-sm font-label-sm text-[11px] uppercase tracking-widest text-on-surface-variant">
          ...
          <span className="ml-2 normal-case tracking-normal text-on-surface-variant">
            <span className="mr-1 inline-block h-2 w-2 rounded-full align-middle" style={{ backgroundColor: QA_SERIES[0].color }} />
            Q
            <span className="mx-1 inline-block h-2 w-2 rounded-full align-middle" style={{ backgroundColor: QA_SERIES[1].color }} />
            A
          </span>
        </p>
