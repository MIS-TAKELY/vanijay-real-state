 ...
 */
export function VerificationStamp({ level, className }: VerificationStampProps) {
  const label = STAMP_LABELS[level] ?? "—";
  const isVerified = level === "LEVEL_2_DOC_VERIFIED" || level === "LEVEL_3_FIELD_VERIFIED";
  ...
  return (
    <span
      ...
      className={cn(
        ...
        !isVerified && !isRejected && "border-outline-variant text-on-surface-variant bg-surface-container",
        className,
      )}
    >
      {label}
