...
interface PropertyCardProps {
  property: {
    ...
    imageUrl?: string;
    meta: string[];
    /** Optional accent badge (e.g. "HOT", "FEATURED") shown top-right. */
    badge?: string;
  };
  className?: string;
}
...
export function PropertyCard({ property, className }: PropertyCardProps) {
  return (
    <article
      data-card
      className={cn(
        "group flex flex-col overflow-hidden rounded-xl border border-outline-variant bg-surface transition-all duration-200 hover:-translate-y-0.5 hover:border-primary/30 hover:shadow-md",
        className,
      )}
    >
      {/* Image Section */}
      ...
      >
        ...
        {property.badge && (
          <Badge
            variant="destructive"
            className="absolute right-2.5 top-2.5 uppercase tracking-wide shadow-sm"
          >
            {property.badge}
          </Badge>
        )}
